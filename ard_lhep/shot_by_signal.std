...
ArducamSDK.Py_ArduCam_setMode(camera.handle,ArducamSDK.EXTERNAL_TRIGGER_MODE)
while(True):
    ...
    aa = ArducamSDK.Py_ArduCam_isFrameReady(camera.handle)
    if aa < 1:
        continue
    print('trigger ', aa)
    # print(ArducamSDK.Py_ArduCam_getSingleFrame(camera.handle))
    # continue
    ...
    try:
        ...
        for i in range(frame_cup):
            ...
            ret, data, cfg = ArducamSDK.Py_ArduCam_getSingleFrame(camera.handle)
            print(ret,data)
            # print('de',ArducamSDK.Py_ArduCam_del(camera.handle))
            if ret != 0:
                print('err ',ret,cfg)
            print('ok   !!!!!!!!!!!!!!!', str(cfg))
