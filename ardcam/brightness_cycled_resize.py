...
while(True):
    ...
    for i in range(frame_cup):
        ...
        print(str(ret)+' '+"%dX%d"%(w1,h1) ) #+str(frame)
        rez_fr = cv2.resize(frame, (w1, h1))
        tm = time.gmtime()
        file_name = str(tm.tm_year) + '-' + str(tm.tm_mon) + '-' + str(tm.tm_mday) + '_' + str(tm.tm_hour) + '-' + str(run_num) + '-' + str(i + 1) + '_' + str(ss) + '_' + str(br)
        cv2.imwrite(file_name+'.jpg', rez_fr)
        ss += frame_inter
#    num+=1
#    if num==10:
#        aaa()
